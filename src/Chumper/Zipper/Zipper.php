...
namespace Epicsagas\Zipper;

use Epicsagas\Zipper\Repositories\RepositoryInterface;
use Exception;
...
class Zipper
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'Epicsagas\Zipper\Repositories\\' . ucwords($type) . 'Repository';
        }

        if (!is_subclass_of($objectOrName, 'Epicsagas\Zipper\Repositories\RepositoryInterface')) {
